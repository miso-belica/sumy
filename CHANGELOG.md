...
## Unreleased
- **FIX:** Raised the `setuptools` lower bound, which at 65.0.0 let a fresh install resolve to a version carrying two advisories. The floor is now 70.0.0, the release that fixes GHSA-cx63-2mw6-8hw5, a ReDoS in `package_index`, and 78.1.1 on Python 3.9+, the release that fixes GHSA-5rjg-fvgr-3xxf, a path traversal in `PackageIndex`. Python 3.8 keeps the 70.0.0 floor and remains exposed to the second one, because 75.3.4 is the last setuptools that installs there.
...
- **FIX:** Raised three dependency lower bounds that never worked. `docopt-ng>=0.6.1` named a version docopt-ng never released (0.6.1 is docopt's), `nltk` 3.9 cannot be imported without the wordnet corpus, and `requests` 2.7.0 fails to import on Python 3.10+. The minimums are now `docopt-ng>=0.6.3`, `nltk>=3.9.1` and `requests>=2.32.4`.
- **FIX:** Gave every remaining dependency a lower bound, so installing sumy alongside an old release no longer silently produces a broken environment. `konlpy` needs 0.6.0, the first release that declares its `JPype1` dependency, and `greek-stemmer-pos` needs 1.1.2 because 1.0.0 is missing the `greek_stemmer.closets` subpackage; `Korean` and `Greek` extras resolving to anything older were already unusable. Bounds were also added for `lxml-html-clean`, `numpy`, `tinysegmenter`, `jieba`, `hebrew_tokenizer`, `pyarabic`, `pythainlp` and `pystempel`.
